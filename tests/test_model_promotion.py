...
def main():
    ...
    if len(versions) < 2:
        ...
        raise Exception("Need at least Version 1 and Version 2")

    current_model = versions[-2]

    candidate_model = versions[-1]

    print("Current Production Candidate:", current_model.version)

    print("New Candidate:", candidate_model.version)
    ...
    candidate_run = client.get_run(candidate_model.run_id)

    current_accuracy = current_run.data.metrics.get("accuracy", 0.0)

    candidate_accuracy = candidate_run.data.metrics.get("accuracy", 0.0)

    print("\nAccuracy comparison")

    print("Version", current_model.version, ":", current_accuracy)

    print("Version", candidate_model.version, ":", candidate_accuracy)
