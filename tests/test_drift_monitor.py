...
def create_columns():
    ...
    return [f"embedding_{i}" for i in range(128)] + [
        "signal_feature_0",
        "signal_feature_1",
        "signal_feature_2",
    ]
...
def create_reference_data():
    ...
    data = np.random.normal(loc=0, scale=1, size=(500, FEATURE_COUNT))

    return pd.DataFrame(data, columns=create_columns())
...
def create_production_data():
    ...
    data = np.random.normal(loc=1.5, scale=2, size=(500, FEATURE_COUNT))
...
def main():
    ...
    monitor = DriftMonitor(drift_threshold=0.3)
