...
class BiosignalPredictor:
    ...
    def __init__(
        ...
        self.model_path = self.model_dir / "classifier.pt"

        self.mapping_path = self.model_dir / "class_mapping.json"

        self.version_path = self.model_dir / "version.txt"
    ...
    def predict(
        ...
        InputValidator.validate(features)
        ...
        predicted_label = self.class_mapping[str(prediction_index)]

        required_confidence = self.config.confidence_thresholds[predicted_label]
        ...
        if label == "Critical":
            ...
            recommended_action = "Review signal quality and patient status"
        ...
        PREDICTION_LABELS.labels(prediction=label).inc()

        CONFIDENCE_SCORE.observe(confidence_value)
